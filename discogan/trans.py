...
def get_data():
    ...
    if args.task_name == 'facescrub':
        ...
        data_A_1, data_A_2 = get_edge2photo_files( item='edges2handbags', test=True )
    ...
    elif args.task_name == 'fashion2nail':
        data_A, test_A = get_edge2photo_files( item='fashion', test=False )
        data_B, test_B = get_edge2photo_files( item='nail', test=True )


    elif args.task_name == 'handbags2nail':
        data_A_1, data_A_2 = get_edge2photo_files( item='handbags', test=False )
        test_A_1, test_A_2 = get_edge2photo_files( item='handbags', test=True )

        data_A = np.hstack( [data_A_1, data_A_2] )
        test_A = np.hstack( [test_A_1, test_A_2] )

        data_B, test_B = get_edge2photo_files( item='nail', test=False )

    elif args.task_name == 'shoes2nail':
        data_A_1, data_A_2 = get_edge2photo_files( item='edges2shoes', test=False )
        test_A_1, test_A_2 = get_edge2photo_files( item='edges2shoes', test=True )

        data_A = np.hstack( [data_A_2] )
        test_A = np.hstack( [test_A_2] )
...
def main():
    ...
    if args.task_name.startswith('edges2'):
        ...
        test_A = read_images( test_style_A, 'A', args.image_size )
        test_B = read_images( test_style_B, 'A', args.image_size )
    ...
    for epoch in range(epoch_size):
        ...
        for i in range(n_batches):
            ...
            if args.task_name.startswith( 'edges2' ):
                ...
                A = read_images( A_path, 'B', args.image_size )
                B = read_images( B_path, 'A', args.image_size )
	    elif args.task_name =='fashion2nail':
                A = read_images( A_path, 'A', args.image_size )
                B = read_images( B_path, 'A', args.image_size )
	    elif args.task_name =='shoes2nail':
